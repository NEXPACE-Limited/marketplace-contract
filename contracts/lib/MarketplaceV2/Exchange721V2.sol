...
import { Exchange } from "./ExchangeV2.sol";
...
abstract contract Exchange721 is Exchange {
    ...
    function isFulfilled(bytes32 orderHash) public view returns (bool) {
        return _fulfill[orderHash] || marketplaceV1.isFulfilled(orderHash);
    }
    ...
    function _atomicMatch721(
        ...
        require(
            _validateSignature(sellerHash, sellerOrder.maker, signatures[0]) ||
                _validateSignatureMarketV1(sellerHash, sellerOrder.maker, signatures[0]),
            "Exchange721/invalidSignature: invalid seller signature"
        );
        require(
            _validateSignature(buyerHash, buyerOrder.maker, signatures[1]) ||
                _validateSignatureMarketV1(buyerHash, buyerOrder.maker, signatures[1]),
            "Exchange721/invalidSignature: invalid buyer signature"
        );
    ...
    function _cancelOrder721(Order721 calldata order, bytes calldata signature) internal {
        ...
        require(
            _validateSignature(orderHash, order.maker, signature) ||
                _validateSignatureMarketV1(orderHash, order.maker, signature),
            "Exchange721/invalidSignature: wrong signature"
        );
        require(isFulfilled(orderHash) == false, "Exchange721/cancelConflict: hash already fulfilled");
    ...
    function _validateOrder721Pair(Order721 calldata sellerOrder, Order721 calldata buyerOrder) internal view {
        ...
        /* fulfilled signature */
        require(isFulfilled(buyerHash) == false, "Exchange721/orderAlreadyUsed: buyer order already used");
        require(isFulfilled(sellerHash) == false, "Exchange721/orderAlreadyUsed: seller order already used");
    }
