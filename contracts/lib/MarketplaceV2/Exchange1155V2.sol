...
abstract contract Exchange1155 is Exchange {
    ...
    function fillsAmounts(bytes32 orderHash) public view returns (uint256) {
        return _fillsAmounts[orderHash] + marketplaceV1.fillsAmounts(orderHash);
    }
    ...
    function _atomicMatch1155(
        ...
        require(fillsAmounts(buyerHash) == 0, "Exchange1155/orderAlreadyUsed: buyer order already used");
        require(
            _validateSignature(buyerHash, buyerOrder.buyerAddress, buyerSignature),
            "Exchange1155/invalidSignature: invalid buyer signature"
        );
        require(
        ...
        for (uint256 i; i < sellerOrders.length; ) {
            ...
            require(
                sellerOrders[i].ftAmounts >= buyerOrder.amounts[i] + fillsAmounts(sellerHash),
            ...
            require(
                _validateSignature(sellerHash, sellerOrders[i].sellerAddress, sellerSignatures[i]) ||
                    _validateSignatureMarketV1(sellerHash, sellerOrders[i].sellerAddress, sellerSignatures[i]),
                "Exchange1155/invalidSignature: invalid seller signature"
            );
    ...
    function _cancelOrder1155(SellerOrder1155 calldata sellerOrder, bytes calldata signature, uint256 value) internal {
        ...
        require(
            _validateSignature(orderHash, sellerOrder.sellerAddress, signature) ||
                _validateSignatureMarketV1(orderHash, sellerOrder.sellerAddress, signature),
            "Exchange1155/invalidSignature: wrong signature"
        );
